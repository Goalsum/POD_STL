...
// TestCase class
class SstreamTest : public TestCase
{
...
void SstreamTest::maxint()
{
    ...
    s << INT_MAX << " " << UINT_MAX << " " << LONG_MAX << " " << ULONG_MAX << " " << INT_MIN << " " << LONG_MIN;
    CPPUNIT_ASSERT(s.good());
...
class full_streambuf : public podstl::streambuf 
{
public:
    typedef podstl::streambuf _Base;
    typedef _Base::int_type int_type;

    full_streambuf(size_t nb_output) : _nb_output(nb_output) {}

    podstl::string const& str() const
    {
        return _buf;
    }
...
void SstreamTest::appate()
{
    ostringstream o("123456789", ios_base::app);
    CPPUNIT_CHECK(o.rdbuf()->pubseekoff(0, ios_base::cur, ios_base::out) == ostringstream::pos_type(9));
    CPPUNIT_CHECK(o.tellp() == ostringstream::pos_type(9));
    o << "abc";
    CPPUNIT_CHECK(o.tellp() == ostringstream::pos_type(12));

    istringstream is("123456789", ios_base::ate);
    CPPUNIT_CHECK(is.rdbuf()->pubseekoff(0, ios_base::cur, ios_base::in) == istringstream::pos_type(9));
    CPPUNIT_CHECK(is.tellg() == istringstream::pos_type(9));
    char c;
    is >> c;
    CPPUNIT_CHECK(is.eof());
    CPPUNIT_CHECK(is.fail());
    CPPUNIT_CHECK(is.tellg() == istringstream::pos_type(-1));
}

void SstreamTest::user_delimeter()
{
    istringstream is("12, -3,0x456789, -07");
